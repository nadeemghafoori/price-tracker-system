...
class Store(object):
    ...
    @classmethod
    def get_by_url_prefix(cls, url_prefix):         # check the user URL with the available store using regular expression
        return cls(**Database.find_one(StoreConstants.COLLECTION, {"url_prefix": {"$regex": '^{}'.format(url_prefix)}}))    # we used the regular expression for matching the url in order to search it using url_prefix
